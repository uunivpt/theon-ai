...
function sortChats(chats: ChatSummary[]) {
  return [...chats].sort((a, b) => {
    const aTime = a.updatedAt && typeof a.updatedAt === "object" && "toMillis" in a.updatedAt ? (a.updatedAt as { toMillis: () => number }).toMillis() : 0;
    const bTime = b.updatedAt && typeof b.updatedAt === "object" && "toMillis" in b.updatedAt ? (b.updatedAt as { toMillis: () => number }).toMillis() : 0;
    return bTime - aTime;
  });
}
...
export default function Home() {
  ...
  function animateResponse(fullText: string) {
    ...
    const frame = (now: number) => {
      const visibleCharacters = Math.min(fullText.length, Math.floor(((now - startedAt) / 1000) * charsPerSecond));
      setMessages((prev) => {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ message: userText, history: previousMessages }),
      });
      ...
      setIsTyping(false);
      setMessages((prev) => [...prev, { role: "ai", text: "I couldn't complete that request. Please try again." }]);
    }
  }
  ...
  return (
    <>
      <MobileLayout>
        <MobileHeader onMenu={() => setMobileSidebarOpen(true)} onSettings={() => router.push("/settings")} />
        <div ref={chatRef} className="flex-1 overflow-y-auto overscroll-contain">
          {messages.length === 0 ? <MobileWelcome /> : <div className="flex flex-col gap-5 px-4 pt-6 pb-36 sm:px-5">
            {messages.map((msg, index) => <ChatBubble key={`${msg.role}-${index}`} role={msg.role} text={msg.text} />)}
            {isTyping && messages[messages.length - 1]?.text === "" && <div className="flex items-end gap-3 px-1"><div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.045]"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-violet-300" /></div></div>}
          </div>}
        </div>
        <MobileInput value={message} onChange={setMessage} onSend={sendMessage} disabled={!userReady || isTyping} />
        <MobileSidebar open={mobileSidebarOpen} onClose={() => setMobileSidebarOpen(false)} chats={chats} currentChatId={currentChatId} onNewChat={startNewChat} onSelectChat={selectChat} onDeleteChat={removeChat} onLogout={logout} />
      </MobileLayout>

      <main className="hidden h-screen bg-gradient-to-br from-black via-[#070707] to-[#111827] md:flex">
        <Sidebar onNewChat={startNewChat} chats={chats} currentChatId={currentChatId} onSelectChat={selectChat} onDeleteChat={removeChat} />
        <section className="flex min-w-0 flex-1 flex-col">
          <Header />
          <div ref={chatRef} className="flex-1 overflow-y-auto overscroll-contain px-8 py-8">
            {messages.length === 0 ? <div className="flex h-full items-center justify-center text-xl text-white/35">Ask me anything...</div> : <div className="mx-auto w-full max-w-3xl space-y-6">{messages.map((msg, index) => <ChatBubble key={`${msg.role}-${index}`} role={msg.role} text={msg.text} />)}</div>}
          </div>
          <footer className="border-t border-white/10 bg-[#090909] p-5">
            <div className="mx-auto flex max-w-3xl gap-3">
              <input value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && sendMessage()} placeholder="Type your message..." disabled={!userReady || isTyping} className="h-16 min-w-0 flex-1 rounded-2xl border border-white/10 bg-white/[0.055] px-6 text-white outline-none placeholder:text-white/25 focus:border-violet-400/40 disabled:opacity-50" />
              <button onClick={sendMessage} disabled={!userReady || isTyping || !message.trim()} className="rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 px-7 font-medium text-white shadow-lg shadow-violet-950/30 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40">Send</button>
            </div>
          </footer>
        </section>
      </main>
