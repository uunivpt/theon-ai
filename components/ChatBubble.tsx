...
function displayAnswer(text: string) { return text.replace(/\n{0,2}(?:#{1,4}\s*)?Sources\s*:\s*[\s\S]*$/i, "").trim(); }

function ChatBubble({ role, text, attachments = [], sources = [], onQuickAction }: ChatBubbleProps) {
  const isUser = role === "user"; const [copied, setCopied] = useState(false); const visibleText = isUser ? text : displayAnswer(text);
  async function copyMessage() { try { await navigator.clipboard.writeText(visibleText); setCopied(true); window.setTimeout(() => setCopied(false), 1800); } catch { setCopied(false); } }
