...
export default function Sidebar({ onNewChat, chats, currentChatId, onSelectChat }: SidebarProps) {
  return (
    <aside className="flex h-full w-[264px] shrink-0 flex-col border-r border-white/[0.08] bg-black p-4">
      ...
      <button onClick={onNewChat} className="mb-5 flex h-11 w-full items-center justify-center gap-2 rounded-2xl border border-white/[0.11] bg-white/[0.045] text-sm font-medium text-white/85 transition hover:border-white/[0.16] hover:bg-white/[0.07] active:scale-[0.985]">
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
        {chats.length === 0 ? (
          ...
          chats.map((chat) => (
            <button key={chat.id} onClick={() => onSelectChat(chat.id)} className={`flex w-full min-w-0 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left transition duration-100 ${currentChatId === chat.id ? "border-white/[0.09] bg-white/[0.07] text-white" : "border-transparent text-white/55 hover:bg-white/[0.045] hover:text-white/80"}`}>
