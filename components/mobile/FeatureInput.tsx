...
function readAsDataUrl(file: Blob): Promise<string> { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(reader.error ?? new Error("Unable to read file")); reader.readAsDataURL(file); }); }
...
export default function FeatureInput({ feature, value, onChange, onSend, onClearFeature, disabled = false }: Props) {
  ...
  async function addFiles(files: FileList | null) {
    if (!files) return; setError(""); setShowPicker(false); const accepted = Array.from(files).slice(0, 4); if (!accepted.length) return; setProcessing(true);
    try { const next = await Promise.all(accepted.map((file) => file.type === "application/pdf" ? extractPdfText(file) : compressImage(file))); setAttachments((current) => [...current, ...next].slice(0, 4)); }
    catch (processingError) { console.error("Attachment processing failed", processingError); setError(processingError instanceof Error ? processingError.message : "Could not process the attachment."); }
  ...
  return <div className="fixed bottom-[max(10px,env(safe-area-inset-bottom))] left-3 right-3 z-30 md:hidden">
    ...
    <div className="relative flex min-h-[60px] items-center gap-2 rounded-[28px] border border-white/[0.12] bg-black px-1.5 py-1.5 shadow-[0_12px_40px_rgba(0,0,0,.65)]"><button type="button" onClick={() => setShowPicker((open) => !open)} disabled={disabled || processing} aria-label="Add attachment" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/[0.10] bg-white/[0.035] text-white/75 active:scale-95 disabled:opacity-40"><Plus size={22} strokeWidth={1.7} /></button><input value={value} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendAttachments(); } }} placeholder={processing ? "Reading PDF..." : feature ? "Add context for Theon..." : "Ask Theon anything..."} disabled={disabled || processing} className="h-11 min-w-0 flex-1 bg-transparent px-1 text-[15px] text-white outline-none placeholder:text-white/30 disabled:opacity-50" /><button onClick={sendAttachments} disabled={!canSend} aria-label="Send message" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 via-purple-500 to-cyan-400 text-black shadow-[0_0_22px_rgba(124,58,237,.28)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-35"><ArrowUp size={21} strokeWidth={2.4} /></button></div>
    {attachments.some((file) => file.type === "application/pdf") && <p className="mt-1.5 text-center text-[9px] text-white/25">🔒 This PDF is used for this analysis and isn&apos;t saved to your chat history.</p>}
  </div>;
}
