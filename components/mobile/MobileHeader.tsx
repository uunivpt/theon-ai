...
export default function MobileHeader({ onMenu, onSettings, onLogout }: Props) {
  ...
  useEffect(() => {
    function handleOutsidePointer(event: PointerEvent) {
      if (profileRef.current && !profileRef.current.contains(event.target as Node)) setProfileOpening(false);
    }
  ...
  return (
    <header className="relative z-20 flex h-[64px] shrink-0 items-center justify-between bg-transparent px-4 pt-1">
      ...
      <div ref={profileRef} className="relative">
        ...
        {profileOpening && (
          <div className="absolute right-0 top-12 w-60 rounded-2xl border border-white/10 bg-[#0b0b0b]/95 p-2 shadow-2xl shadow-black/50 backdrop-blur-xl">
            <div className="px-3 py-2">
              <p className="text-[10px] uppercase tracking-[0.16em] text-white/25">Profile</p>
              <p className="mt-1 truncate text-xs text-white/60">{email}</p>
            </div>
            <div className="my-1 h-px bg-white/[0.07]" />
            <div className="grid grid-cols-2 gap-1 px-1">
              <button onClick={() => changeTheme("dark")} className={`flex items-center justify-center gap-1.5 rounded-lg py-2 text-[10px] ${theme === "dark" ? "bg-white/10 text-white" : "text-white/35"}`}><Moon size={12} />Dark</button>
              <button onClick={() => changeTheme("light")} className={`flex items-center justify-center gap-1.5 rounded-lg py-2 text-[10px] ${theme === "light" ? "bg-white/10 text-white" : "text-white/35"}`}><Sun size={12} />Light</button>
            </div>
