...
export default function MobileSidebar({ open, onClose, chats, currentChatId, onNewChat, onSelectChat, onLogout }: Props) {
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto px-4">
          ...
          {chats.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {chats.map((chat) => (
                <button key={chat.id} onClick={() => onSelectChat(chat.id)} className={`flex w-full items-center gap-3 rounded-xl border px-3.5 py-3 text-left transition ${currentChatId === chat.id ? "border-white/[0.08] bg-white/[0.07] text-white" : "border-transparent text-white/55 hover:bg-white/[0.045]"}`}>
                  <MessageSquare size={16} className="shrink-0 opacity-45" />
                  <span className="min-w-0 flex-1"><span className="block truncate text-[13px]">{chat.title}</span>{chat.preview && <span className="mt-0.5 block truncate text-[10px] text-white/25">{chat.preview}</span>}</span>
                </button>
              ))}
            </div>
          )}
