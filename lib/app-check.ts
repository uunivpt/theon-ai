...
export async function verifyFirebaseAppCheck(token: string, consume = false) {
  ...
  const response = await fetch(`${VERIFY_BASE}/${encodeURIComponent(project)}:verifyAppCheckToken`, {
    ...
    body: JSON.stringify({ appCheckToken: token, ...(consume ? { consumeAppCheckToken: true } : {}) }),
    cache: "no-store",
    signal: AbortSignal.timeout(5000),
  });
  if (!response.ok) {
  ...
  const payload = await response.json() as { token?: AppCheckClaims; alreadyConsumed?: boolean };
  if (!payload.token) throw new Error("Invalid App Check token response.");
  if (consume && payload.alreadyConsumed) throw new Error("Replayed App Check token.");
  return payload.token;
}

export async function requireFirebaseAppCheck(request: Request, options?: { consume?: boolean }) {
  const token = request.headers.get("x-firebase-appcheck")?.trim() || "";
  if (!token) throw new AppCheckError("App verification required.", 401);
  try {
    return await verifyFirebaseAppCheck(token, options?.consume === true);
  } catch {
    throw new AppCheckError("App verification failed.", 401);
  }
}
