...
export type AuthenticatedUser = { uid: string; email?: string; emailVerified?: boolean };

function firebaseApiKey() { return process.env.FIREBASE_WEB_API_KEY || process.env.NEXT_PUBLIC_FIREBASE_API_KEY || ""; }
...
export async function requireFirebaseUser(request: Request): Promise<AuthenticatedUser> {
  ...
  if (!response.ok || !Array.isArray(payload?.users) || !payload.users[0]?.localId) throw new AuthError("Invalid or expired session.", 401);
  ...
  if (process.env.FIREBASE_APPCHECK_ENFORCE === "true") {
    try { await requireFirebaseAppCheck(request); }
    catch (error) {
  ...
  return { uid: String(claims.sub), email: typeof user.email === "string" ? user.email : undefined, emailVerified: user.emailVerified === true };
}
