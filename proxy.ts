...
const API_BODY_LIMITS: Record<string, number> = { "/api/chat": 16 * 1024 * 1024, "/api/generate-image": 256 * 1024 };
const ALLOWED_ORIGINS = new Set(["https://theonai.online", "https://www.theonai.online", "http://localhost:3000", "http://127.0.0.1:3000"]);
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith("/api/")) {
    ...
    if (fetchSite && !["same-origin", "same-site", "none"].includes(fetchSite)) return secure(NextResponse.json({ error: "Cross-site API requests are not allowed." }, { status: 403 }), requestId);
    if (request.method !== "POST") return secure(NextResponse.json({ error: "Method not allowed." }, { status: 405, headers: { Allow: "POST" } }), requestId);

    const maxBody = API_BODY_LIMITS[pathname] || 2 * 1024 * 1024;
    const contentLength = requestContentLength(request);
    if (contentLength !== null && contentLength > maxBody) return secure(NextResponse.json({ error: "Request is too large." }, { status: 413 }), requestId);

    const auth = request.headers.get("authorization") || "";
    if (!/^Bearer\s+[^\s]+$/i.test(auth)) return secure(NextResponse.json({ error: "Authentication required." }, { status: 401 }), requestId);
    if (process.env.FIREBASE_APPCHECK_ENFORCE === "true" && !request.headers.get("x-firebase-appcheck")) return secure(NextResponse.json({ error: "App verification required." }, { status: 401 }), requestId);

    const ip = getClientIp(request);
    const perIpLimit = pathname === "/api/generate-image" ? 6 : 20;
    const localRate = consumeRateLimit(`${pathname}:${ip}`, perIpLimit, 10_000);
    if (!localRate.allowed) return secure(NextResponse.json({ error: "Too many requests. Please slow down." }, { status: 429, headers: { "Retry-After": String(localRate.retryAfter) } }), requestId);
